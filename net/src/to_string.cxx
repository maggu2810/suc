...
{
    static_assert((AF == AF_INET && SZ >= INET_ADDRSTRLEN) ||
                  (AF == AF_INET6 && SZ >= INET6_ADDRSTRLEN));

    char buf[SZ];
    if (::inet_ntop(AF, &address, buf, sizeof(buf)) == nullptr)
    {
        const auto errnum = errno;
        throw std::runtime_error(std::format("Failed to convert address to string (expected "
                                             "because we supply correct values) [errno: {}]",
                                             errnum));
    }
    return {buf};
}
...
std::string to_string(const inaddr_storage& address)
{
    ...
    return std::visit(
        suc::cmn::overloaded {
            ...
            [](const in6_addr& arg) -> R
            {
                return to_string(arg);
            },
            [](const std::monostate& arg) -> R
            {
...
std::string to_string(const sockaddr& sa, socklen_t addrlen)
{
    ...
    {
        const auto errnum = errno;
        throw std::runtime_error(std::format("getnameinfo failed [errno: {}]", errnum));
    }
    std::stringstream ss;
