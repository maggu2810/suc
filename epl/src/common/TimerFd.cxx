...
TimerFd::TimerFd(std::function<void(uint64_t)> func, EventQueue& eventQueue) : TimerFd(eventQueue)
{
    onShot(std::move(func));
}

TimerFd::TimerFd(const itimerspec&             value,
...
{
    // If there are unread expirations, this will not lead to a read anymore and on a new set the
    // expirations starts from zero. So set time to zeroes is enough, we do not need to read the
    // expirations.
    lowLevelSetTime({});
}
