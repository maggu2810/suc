    ...
    switch (eventType)
    {
        ...
        default:
            throw std::runtime_error(
                std::format("unknown event type: {}", std::to_underlying(eventType)));
    }
}
...
{
    // potentially a callback can modify m_fds, so we cannot fetch the member by a key once and use
    // that for all callbacks in one run.
    auto it = m_fds.find(fd);
    if (it == m_fds.end())
    {
        // If there is no entry, we call no function and so, the entry cannot be present for the
        // next event type.
        return;
    }
    ...
    for (const auto& eventType : kEventTypes)
    {
        ...
        {
            // If there is no entry, we call no function and so, the entry cannot be present for the
            // next event type, so we could break here.
            return;
        }
    }
}
