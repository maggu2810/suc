...
concept ValidEndian =
    requires(std::endian&& e) { e == std::endian::little || e == std::endian::big; };
...
constexpr void copy(void* dst, const void* src, std::size_t length)
{
    ...
    {
        std::reverse_copy(static_cast<const char*>(src),
...
{
    impl::copy<impl::direction<endian>()>(static_cast<char*>(dst),
